...
class TestRequestContextMiddleware:
    async def test_generates_request_id_header_when_absent(self, client: AsyncClient) -> None:
        resp = await client.get("/ping")
        assert resp.status_code == 200
        assert resp.headers.get("x-request-id")

    async def test_request_id_available_inside_handler(self, client: AsyncClient) -> None:
        resp = await client.get("/ping")
        assert resp.json()["request_id"] == resp.headers["x-request-id"]

    async def test_reuses_client_supplied_request_id(self, client: AsyncClient) -> None:
    ...
    async def test_rejects_malformed_client_request_id(self, client: AsyncClient) -> None:
        bad = "x" * 500 + "\n injected"
...
class TestAccessLogMiddleware:
    async def test_emits_access_log_with_required_fields(self, client: AsyncClient) -> None:
