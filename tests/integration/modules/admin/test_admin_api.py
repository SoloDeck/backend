...
class TestAdminCreateTemplate:
    ...
    async def test_loc_thu_vien_theo_nghe(
        ...
        await client.post(
            "/api/v1/admin/templates",
            json=self._template_payload(name="Mẫu content", profession="content-writer"),
            headers=headers,
        )
        await client.post(
            "/api/v1/admin/templates",
        ...
        resp = await client.get(
            "/api/v1/admin/templates?profession=content-writer", headers=headers
        )
        assert resp.status_code == 200
        data = resp.json()["data"]
        assert all(t["profession"] == "content-writer" for t in data)
