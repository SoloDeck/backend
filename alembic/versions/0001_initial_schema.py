"""Initial schema — all tables, enums, triggers, indexes.
...
def upgrade() -> None:
    ...
    # Enum types
    # ------------------------------------------------------------------
    _exec("CREATE TYPE user_role AS ENUM ('freelancer', 'admin')")
    _exec("CREATE TYPE user_status AS ENUM ('active', 'suspended', 'deleted')")
    _exec("CREATE TYPE notification_channel AS ENUM ('email', 'in_app', 'both', 'zalo')")
    _exec("CREATE TYPE theme_preference AS ENUM ('light', 'dark')")

    _exec("CREATE TYPE subscription_status AS ENUM ('active', 'past_due', 'suspended', 'cancelled')")
    _exec(
    ...
    _exec("CREATE TYPE client_type AS ENUM ('individual', 'company')")
    _exec("CREATE TYPE client_status AS ENUM ('prospect', 'active', 'inactive', 'archived')")
    _exec("CREATE TYPE comm_channel AS ENUM ('email', 'phone', 'meeting', 'message', 'zalo')")
    ...
    _exec("CREATE TYPE deal_source AS ENUM ('inbound', 'referral', 'outreach', 'platform', 'other')")
    _exec(
        "CREATE TYPE deal_activity_type AS ENUM ("
        "  'stage_change', 'note_added', 'document_attached', 'ai_qualification'"
        ")"
    )
    _exec("CREATE TYPE ai_recommendation AS ENUM ('qualify', 'pass')")
    _exec("CREATE TYPE lead_score_level AS ENUM ('hot', 'warm', 'cold')")
    ...
    _exec(
        ...
        "  'follow_up', 'proposal_follow_up', 'contract_signing_nudge',"
        "  'payment_due', 'payment_overdue', 're_engagement', 'custom'"
        ")"
    )
    _exec(
        "CREATE TYPE reminder_status AS ENUM ('pending', 'sent', 'failed', 'cancelled', 'skipped')"
    )
    _exec("CREATE TYPE reminder_outcome AS ENUM ('success', 'failure')")

    _exec("CREATE TYPE template_type AS ENUM ('proposal', 'contract', 'clause')")
    ...
    _exec("CREATE TYPE ai_generation_status AS ENUM ('pending', 'completed', 'failed')")
    _exec("CREATE TYPE period_type AS ENUM ('monthly', 'quarterly', 'yearly')")
    ...
    _exec("CREATE INDEX idx_users_status_deleted ON users (status, deleted_at)")
    _exec(
        """
    ...
    _exec("CREATE INDEX idx_oauth_identities_user_id ON oauth_identities (user_id)")
    ...
    _exec("CREATE INDEX idx_token_blacklist_expires ON token_blacklist (expires_at)")
    ...
    _exec("CREATE INDEX idx_subscription_plans_active ON subscription_plans (is_active)")
    _exec(
        """
    ...
    _exec("CREATE INDEX idx_subscriptions_plan_status ON subscriptions (plan_id, status)")
    _exec("CREATE INDEX idx_subscriptions_period_end  ON subscriptions (current_period_end)")
    _exec(
        """
    ...
    _exec("CREATE INDEX idx_usage_records_subscription ON usage_records (subscription_id)")
    _exec(
        """
    ...
    _exec(
        ...
        CREATE TABLE billing_events (
            ...
            CONSTRAINT fk_billing_events_user         FOREIGN KEY (user_id)
                REFERENCES users (id),
            CONSTRAINT fk_billing_events_subscription FOREIGN KEY (subscription_id)
                REFERENCES subscriptions (id),
            CONSTRAINT uq_billing_events_stripe UNIQUE (stripe_event_id)
        )
        """
    )
    _exec("CREATE INDEX idx_billing_events_user ON billing_events (user_id, occurred_at DESC)")
    _exec("CREATE INDEX idx_billing_events_subscription ON billing_events (subscription_id)")
    ...
    _exec("CREATE INDEX idx_clients_owner_status  ON clients (owner_user_id, status)")
    _exec("CREATE INDEX idx_clients_owner_deleted ON clients (owner_user_id, deleted_at)")
    _exec(
    ...
    _exec("CREATE INDEX idx_client_tags_client ON client_tags (client_id)")
    ...
    _exec(
        ...
        CREATE TABLE client_communication_logs (
            ...
            CONSTRAINT fk_client_comm_logs_client FOREIGN KEY (client_id)
                REFERENCES clients (id),
            CONSTRAINT fk_client_comm_logs_user FOREIGN KEY (owner_user_id)
                REFERENCES users (id)
        )
        """
    )
    _exec(
        "CREATE INDEX idx_client_comm_logs_client"
        " ON client_communication_logs (client_id, communicated_at DESC)"
    )
    _exec("CREATE INDEX idx_client_comm_logs_user ON client_communication_logs (owner_user_id)")
    ...
    _exec(
        ...
        CREATE TABLE deals (
            ...
            CONSTRAINT chk_deals_qualification_score
                CHECK (ai_qualification_score IS NULL
                       OR ai_qualification_score BETWEEN 0 AND 100)
        )
        """
    )
    _exec("CREATE INDEX idx_deals_owner_stage   ON deals (owner_user_id, stage)")
    _exec("CREATE INDEX idx_deals_owner_deleted ON deals (owner_user_id, deleted_at)")
    _exec("CREATE INDEX idx_deals_client        ON deals (client_id)")
    _exec("CREATE INDEX idx_deals_owner_created ON deals (owner_user_id, created_at DESC)")
    _exec("CREATE INDEX idx_deals_stage_closed  ON deals (stage, closed_at)")
    _exec(
        """
    ...
    _exec(
        ...
        CREATE TABLE deal_activity_entries (
            ...
            CONSTRAINT fk_deal_activity_entries_deal FOREIGN KEY (deal_id)
                REFERENCES deals (id),
            CONSTRAINT fk_deal_activity_entries_user FOREIGN KEY (owner_user_id)
                REFERENCES users (id)
        )
        """
    )
    _exec(
        "CREATE INDEX idx_deal_activity_entries_deal"
        " ON deal_activity_entries (deal_id, created_at DESC)"
    )
    _exec("CREATE INDEX idx_deal_activity_entries_user ON deal_activity_entries (owner_user_id)")
    ...
    _exec("CREATE INDEX idx_proposals_deal_status   ON proposals (deal_id, status)")
    _exec("CREATE INDEX idx_proposals_owner_status  ON proposals (owner_user_id, status)")
    _exec(
    ...
    _exec(
        ...
        CREATE TABLE contracts (
            ...
            CONSTRAINT fk_contracts_parent   FOREIGN KEY (parent_contract_id)
    ...
    _exec("CREATE INDEX idx_contracts_deal_status   ON contracts (deal_id, status)")
    _exec("CREATE INDEX idx_contracts_owner_status  ON contracts (owner_user_id, status)")
    _exec("CREATE INDEX idx_contracts_client        ON contracts (client_id)")
    _exec("CREATE INDEX idx_contracts_proposal      ON contracts (proposal_id)")
    _exec(
        """
    ...
    _exec(
        ...
        " ON contract_payment_milestones (contract_id, sort_order)"
    )
    _exec(
        """
    ...
    _exec("CREATE INDEX idx_invoices_owner_status   ON invoices (owner_user_id, status)")
    _exec("CREATE INDEX idx_invoices_owner_due_date ON invoices (owner_user_id, due_date)")
    _exec(
        "CREATE INDEX idx_invoices_owner_issued ON invoices (owner_user_id, issue_date DESC)"
    )
    _exec("CREATE INDEX idx_invoices_client         ON invoices (client_id)")
    _exec(
        "CREATE INDEX idx_invoices_contract ON invoices (contract_id)"
        " WHERE contract_id IS NOT NULL"
    )
    _exec("CREATE INDEX idx_invoices_deal ON invoices (deal_id) WHERE deal_id IS NOT NULL")
    _exec(
        """
    ...
    # Back-fill FK from milestones to invoices
    _exec(
    ...
    _exec(
        ...
        CREATE TABLE invoice_payment_records (
            ...
            CONSTRAINT fk_invoice_payment_records_invoice FOREIGN KEY (invoice_id)
                REFERENCES invoices (id),
    ...
    _exec(
        ...
        CREATE TABLE reminders (
            ...
            CONSTRAINT fk_reminders_user   FOREIGN KEY (owner_user_id)
                REFERENCES users (id),
            CONSTRAINT fk_reminders_parent FOREIGN KEY (parent_reminder_id)
    ...
    _exec("CREATE INDEX idx_reminders_target ON reminders (target_type, target_id)")
    _exec(
    ...
    _exec(
        "CREATE INDEX idx_revenue_snapshots_owner"
        " ON revenue_snapshots (owner_user_id, period_type, period_start DESC)"
    )
    ...
    _exec(
        "CREATE INDEX idx_pipeline_snapshots_owner"
        " ON pipeline_snapshots (owner_user_id, snapshot_date DESC)"
    )
    ...
    _exec("CREATE INDEX idx_audit_log_actor  ON audit_log_entries (actor_user_id, occurred_at DESC)")
    _exec(
        "CREATE INDEX idx_audit_log_target"
        " ON audit_log_entries (target_type, target_id, occurred_at DESC)"
    )
    _exec("CREATE INDEX idx_audit_log_event ON audit_log_entries (event_type, occurred_at DESC)")
    ...
    _exec(
        ...
        CREATE TABLE system_templates (
            ...
            CONSTRAINT fk_system_templates_admin  FOREIGN KEY (created_by_admin_id)
                REFERENCES users (id),
            CONSTRAINT fk_system_templates_parent FOREIGN KEY (parent_template_id)
                REFERENCES system_templates (id),
    ...
    _exec(
        ...
        CREATE TABLE ai_cost_records (
            ...
            CONSTRAINT chk_ai_cost_records_tokens
                CHECK (input_tokens >= 0 AND output_tokens >= 0),
            CONSTRAINT chk_ai_cost_records_cost CHECK (estimated_cost_usd >= 0)
        )
        """
    )
    _exec("CREATE INDEX idx_ai_cost_records_user   ON ai_cost_records (user_id, occurred_at DESC)")
    _exec("CREATE INDEX idx_ai_cost_records_module ON ai_cost_records (ai_module, occurred_at DESC)")
    _exec("CREATE INDEX idx_ai_cost_records_time   ON ai_cost_records (occurred_at DESC)")
...
def downgrade() -> None:
    # Admin domain
    _exec("DROP TABLE IF EXISTS ai_cost_records CASCADE")
    _exec("DROP TABLE IF EXISTS feature_flags CASCADE")
    _exec("DROP TABLE IF EXISTS system_templates CASCADE")
    _exec("DROP TABLE IF EXISTS audit_log_entries CASCADE")

    # Analytics
    _exec("DROP TABLE IF EXISTS pipeline_snapshots CASCADE")
    _exec("DROP TABLE IF EXISTS revenue_snapshots CASCADE")

    # Reminders
    _exec("DROP TABLE IF EXISTS reminder_delivery_records CASCADE")
    _exec("DROP TABLE IF EXISTS reminders CASCADE")

    # Invoices
    _exec("DROP TABLE IF EXISTS invoice_payment_records CASCADE")
    _exec("DROP TABLE IF EXISTS invoice_line_items CASCADE")
    _exec("DROP TABLE IF EXISTS invoices CASCADE")

    # Contracts
    _exec("DROP TABLE IF EXISTS contract_payment_milestones CASCADE")
    _exec("DROP TABLE IF EXISTS contracts CASCADE")

    # Proposals
    _exec("DROP TABLE IF EXISTS proposals CASCADE")

    # Deals
    _exec("DROP TABLE IF EXISTS deal_activity_entries CASCADE")
    _exec("DROP TABLE IF EXISTS deals CASCADE")

    # Clients
    _exec("DROP TABLE IF EXISTS client_communication_logs CASCADE")
    _exec("DROP TABLE IF EXISTS client_tags CASCADE")
    _exec("DROP TABLE IF EXISTS clients CASCADE")

    # Subscriptions
    _exec("DROP TABLE IF EXISTS billing_events CASCADE")
    _exec("DROP TABLE IF EXISTS usage_records CASCADE")
    _exec("DROP TABLE IF EXISTS subscriptions CASCADE")
    _exec("DROP TABLE IF EXISTS subscription_plans CASCADE")

    # Identity
    _exec("DROP TABLE IF EXISTS token_blacklist CASCADE")
    _exec("DROP TABLE IF EXISTS password_reset_tokens CASCADE")
    _exec("DROP TABLE IF EXISTS refresh_tokens CASCADE")
    _exec("DROP TABLE IF EXISTS oauth_identities CASCADE")
    _exec("DROP TABLE IF EXISTS users CASCADE")

    # Trigger function
    _exec("DROP FUNCTION IF EXISTS set_updated_at CASCADE")

    # Enum types
    for enum_type in [
        "period_type", "ai_generation_status", "ai_module_type",
        "template_type", "reminder_outcome", "reminder_status",
        "reminder_type_enum", "reminder_target_type",
        "payment_method", "invoice_status", "contract_status",
        "proposal_status", "lead_score_level", "ai_recommendation",
        "deal_activity_type", "deal_source", "deal_stage",
        "comm_channel", "client_status", "client_type",
        "billing_event_type", "subscription_status",
        "theme_preference", "notification_channel",
        "user_status", "user_role",
    ]:
        _exec(f"DROP TYPE IF EXISTS {enum_type} CASCADE")
