...
def build_downgrade_email(
    *,
    owner_name: str | None,
    plan_name: str,
    plan_url: str | None = None,
) -> EmailContent:
    """Gói trả phí đã hết hạn và tài khoản vừa được hạ về Free."""
    greeting = (owner_name or "").strip() or "bạn"
    subject = "[SoloDesk] Tài khoản đã chuyển về gói Miễn phí"

    plain = (
        f"Chào {greeting},\n\n"
        f"Gói {plan_name} của bạn đã hết hạn, nên tài khoản đã tự chuyển về gói Miễn phí. "
        "Một số tính năng AI có thể bị giới hạn.\n\n"
        "Bạn có thể nâng cấp lại bất cứ lúc nào để mở lại đầy đủ tính năng.\n\n"
        "— SoloDesk"
    )
    html = _wrap_html(
        f"<p>Chào {escape(greeting)},</p>"
        f"<p>Gói <strong>{escape(plan_name)}</strong> của bạn đã hết hạn, nên tài khoản "
        "đã tự chuyển về gói <strong>Miễn phí</strong>. Một số tính năng AI có thể bị "
        "giới hạn.</p>"
        "<p>Bạn có thể nâng cấp lại bất cứ lúc nào để mở lại đầy đủ tính năng.</p>",
        plan_url,
    )
    return EmailContent(subject=subject, html=html, plain=plain)
