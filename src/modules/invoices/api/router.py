...
async def send_invoice(invoice_id: uuid.UUID, user_id: CurrentUserId, db: DBSession) -> ApiResponse[InvoiceResponse]:
...
async def void_invoice(invoice_id: uuid.UUID, user_id: CurrentUserId, db: DBSession) -> ApiResponse[InvoiceResponse]:
    invoice = await InvoicesService(db=db).void(user_id, invoice_id)
    return ApiResponse.ok(InvoiceResponse.model_validate(invoice))


@router.post("/{invoice_id}/payments", response_model=ApiResponse[InvoiceResponse], status_code=status.HTTP_201_CREATED)
async def record_payment(invoice_id: uuid.UUID, payload: PaymentRequest, user_id: CurrentUserId, db: DBSession) -> ApiResponse[InvoiceResponse]:
...
async def list_payments(invoice_id: uuid.UUID, user_id: CurrentUserId, db: DBSession) -> ApiResponse[list[PaymentRecordResponse]]:
