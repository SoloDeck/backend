...
class ClientsRepository:
    ...
    async def list_all(self, owner_user_id: uuid.UUID, status: str | None = None, name: str | None = None, email: str | None = None) -> list:
        conditions = [ClientModel.owner_user_id == owner_user_id, ClientModel.deleted_at.is_(None)]
        if status is not None: conditions.append(ClientModel.status == status)
        if name is not None: conditions.append(ClientModel.name.ilike(f"%{name}%"))
        if email is not None: conditions.append(ClientModel.email.ilike(f"%{email}%"))
    ...
    async def list_comm_logs(self, owner_user_id: uuid.UUID, client_id: uuid.UUID) -> list:
        result = await self.db.execute(select(ClientCommunicationLogModel).where(ClientCommunicationLogModel.client_id == client_id, ClientCommunicationLogModel.owner_user_id == owner_user_id))
        return list(result.scalars().all())
