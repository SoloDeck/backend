...
class BaseAIChain(ABC):
    ...
    async def get_provider(self):
        """
        Retrieve the currently configured LLM provider.
        ProviderFactory handles provider + model selection.
        """
        if self._provider is None:
            self._provider = await ProviderFactory(
                self.db
            ).get_provider()
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            raise AIGenerationError(
                f"{self.module_name} generation failed: {exc}"
            ) from exc